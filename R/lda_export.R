...
#' @param output_file Base path/prefix for output file, without extension. If NULL, does not save (default: NULL)
#' @param format Output format: "xlsx", "csv", or "rda" (default: "xlsx")
#'
...
#'
#' @export
get_top_words <- function(model, n_words = 30, output_file = NULL, format = "xlsx") {

  if (!inherits(model, "LDA")) {
    stop("model must be an LDA object from the topicmodels package")
  }

  format <- match.arg(format, c("xlsx", "csv", "rda"))
  ...
  if (!is.null(output_file)) {
    out_path <- paste0(output_file, ".", format)

    if (format == "xlsx") {
      openxlsx::write.xlsx(topwords, out_path)
    } else if (format == "csv") {
      utils::write.csv(topwords, out_path, row.names = FALSE)
    } else if (format == "rda") {
      top_words <- topwords
      save(top_words, file = out_path)
    }

    message("Top words saved to: ", out_path)
  }
...
#'
#' @export
get_frex_words <- function(model, n_words = 30, weight = 0.3, output_file = NULL, format = "xlsx") {
  ...
  if (!is.null(output_file)) {
    ...
    if (format == "xlsx") {
      openxlsx::write.xlsx(topFREX, out_path)
    } else if (format == "csv") {
      utils::write.csv(topFREX, out_path, row.names = FALSE)
    } else if (format == "rda") {
      frex_words <- topFREX
      save(frex_words, file = out_path)
    }

    message("FREX words saved to: ", out_path)
  }
...
get_top_docs <- function(model, doc_data, index_col = "index", text_col = "text",
                         n_docs = 30, output_file = NULL, format = "xlsx") {
  ...
  if (!is.null(output_file)) {
    ...
    if (format == "xlsx") {
      openxlsx::write.xlsx(top_texts_wide, out_path)
    } else if (format == "csv") {
      utils::write.csv(top_texts_wide, out_path, row.names = FALSE)
    } else if (format == "rda") {
      top_docs <- top_texts_wide
      save(top_docs, file = out_path)
    }

    message("Top documents saved to: ", out_path)
  }
...
#' @param format Output format for all three files: "xlsx", "csv", or "rda" (default: "xlsx")
#'
...
export_lda_results <- function(model,
  ...
  # Create output file base paths (no extension - added inside each get_* fn)
  words_file <- file.path(output_dir, paste0(output_prefix, "_TopWords_k", k))
  frex_file <- file.path(output_dir, paste0(output_prefix, "_TopFREX_k", k))
  docs_file <- file.path(output_dir, paste0(output_prefix, "_TopDocs_k", k))

  message("Exporting results for k = ", k)

  # Export top words
  message("Getting top words...")
  top_words <- get_top_words(model, n_words = n_words, output_file = words_file, format = format)

  # Export FREX words
  message("Getting FREX words...")
  frex_words <- get_frex_words(model, n_words = n_words, output_file = frex_file, format = format)
  ...
  top_docs <- get_top_docs(model, doc_data = doc_data,
                           ...
                           n_docs = n_docs, output_file = docs_file, format = format)
